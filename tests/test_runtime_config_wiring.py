...
def test_streamed_meta_materialization_initializes_every_parameter():
    ...
    adc._materialize_meta_module_streamed(
        module,
    ...
    assert torch.count_nonzero(parameters[0]) > 0


def test_streamed_student_initialization_uses_xavier_and_zero_biases():
    with torch.device("meta"):
        module = torch.nn.Sequential(
            torch.nn.Linear(5, 9),
            torch.nn.Linear(9, 2),
