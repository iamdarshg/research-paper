...
To improve the physical plausibility of the generated geometries, we integrate the CFD solver into the generative optimization loop. In this implementation, we utilize the LBM solver to compute aerodynamic loss gradients (via finite-difference or surrogate-proxy approximations) or as a score-guidance function during the reverse diffusion process. This approach provides a first-order approximation of the aerodynamic manifold within the latent space, guiding the model toward aerodynamically efficient forms.
