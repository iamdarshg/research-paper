...
The results in Table \ref{tab:cd_validation} demonstrate that following two critical physics corrections---correction of the freestream lattice velocity to $u = Ma / \sqrt{3}$ and standardization of the BFL weighting factor to $4.0/(1+2q)$---the solver provides $C_d$ values that align with physical expectations. Specifically, the cylinder result is within 15\% of the typical range for low-Reynolds flow, and the cube result is in excellent agreement with standard values. These calibrated physics enable the generation of reliable aerodynamic labels for surrogate training within our framework.
