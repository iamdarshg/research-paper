...
Throughout this work, we developed and compared three complementary CFD evaluation strategies. This section synthesizes the trade-offs and recommendations.

\subsection{CFD Method Comparison}

\subsubsection{Surrogate Model (Fast Physics-Based)}
\begin{itemize}
    \item \textbf{Method}: Lifting line theory with Schlichting viscous drag
    \item \textbf{Speed}: 0.1 s per design (GPU-vectorized)
    \item \textbf{Accuracy}: MAE = 0.89 m on range (vs CFD), $\rho_{\text{Spearman}} = 0.93$ ranking
    \item \textbf{Memory}: <100 MB
    \item \textbf{Pros}: Zero installation, always available, ranking preserves RL exploration
    \item \textbf{Cons}: ±20\% error, brittle outside classical assumptions (e.g., non-planar folds)
    \item \textbf{Best for}: Optimization loops, design space exploration, rapid iteration
\end{itemize}

\subsubsection{FluidX3D (High-Fidelity CFD)}
\begin{itemize}
    \item \textbf{Method}: GPU-accelerated Lattice Boltzmann (D3Q27 lattice, 5000 iterations)
    \item \textbf{Speed}: 10-20 s per design (RTX 3090, depending on mesh complexity)
    \item \textbf{Accuracy}: ±3-5\% error, validated against classical aerodynamic data
    \item \textbf{Memory}: 2-4 GB VRAM
    \item \textbf{Pros}: Windows-native (no Docker), physically grounded, discovers non-planar effects
    \item \textbf{Cons}: Requires GPU installation, slower, not suitable for 1000+ design screening
    \item \textbf{Best for}: Final validation, design ranking, sensitivity analysis
\end{itemize}

\subsubsection{Hybrid Multi-Fidelity (Auto-Select)}
\begin{itemize}
    \item \textbf{Strategy}: Use surrogate for screening, FluidX3D for promising designs
    \item \textbf{Avg Speed}: 5 s per design (dominated by 50 high-fidelity calls)
    \item \textbf{Accuracy}: 90\% (weighted by call frequency)
    \item \textbf{Compute Reduction}: 98\% vs pure FluidX3D (500 k steps × 10 s vs 50 k × 1 ms + 50 × 10 s)
    \item \textbf{Pros}: Balanced accuracy and speed, asymptotic convergence to CFD
    \item \textbf{Cons}: Requires tuning threshold (0.9 × target L/D)
    \item \textbf{Best for}: Production optimization, when both speed and accuracy matter
\end{itemize}

\subsection{Training Method Comparison}

\subsubsection{DDPG (Reinforcement Learning)}
\begin{itemize}
    \item \textbf{Algorithm}: Deep Deterministic Policy Gradient with Ornstein-Uhlenbeck exploration
    \item \textbf{Training Time}: 100 episodes × 1-10 s per eval = 5-10 minutes (surrogate) / 1+ hours (CFD)
    \item \textbf{Convergence}: Asymptotic reward improvement, plateaus after 50-100 episodes
    \item \textbf{Discovered Designs}: Single locally-optimal fold pattern
    \item \textbf{Pros}: Direct, interpretable objective (range), efficient RL loops
    \item \textbf{Cons}: Exploits single design, no pattern generalization
    \item \textbf{Best for}: Single-objective optimization, rapid prototyping
\end{itemize}

\subsubsection{Recursive GNN (Pattern Learning)}
\begin{itemize}
    \item \textbf{Algorithm}: 4-layer GIN with 3 recursive levels, multi-head attention
    \item \textbf{Training Time}: 50 epochs on synthetic data = 2-5 minutes
    \item \textbf{Generalization}: Learned mesh $\to$ aerodynamics mapping, transfers to new folds
    \item \textbf{Interpretability}: Attention weights reveal which fold pairs matter
    \item \textbf{Pros}: Pattern discovery, transfer learning, explains design principles
    \item \textbf{Cons}: Requires 2000 synthetic CFD samples for pre-training (50 GPU-hours), may overfit
    \item \textbf{Best for}: Understanding fold-aerodynamic relationships, meta-learning, design theory
\end{itemize}

\subsection{Workflow Recommendations}

\subsubsection{Fast Exploration (2 minutes)}
\begin{enumerate}
    \item Use surrogate model + DDPG agent
    \item Train for 30 episodes
    \item Outcome: Quick baseline design, verify system works
\end{enumerate}

\subsubsection{Balanced Optimization (5-10 minutes)}
\begin{enumerate}
    \item Use hybrid method + DDPG agent
    \item Train for 50 episodes (most evaluations via surrogate, ~50 via FluidX3D)
    \item Outcome: Optimized design validated by CFD
\end{enumerate}

\subsubsection{High-Fidelity Validation (20 minutes)}
\begin{enumerate}
    \item Use surrogate for exploration (100 episodes)
    \item Then run top 5 designs through FluidX3D
    \item Outcome: Ensemble of validated designs, final selection via CFD
\end{enumerate}

\subsubsection{Pattern Discovery (10 minutes)}
\begin{enumerate}
    \item Train GNN on surrogate-generated data
    \item Analyze attention patterns for design insights
    \item Use GNN to predict performance of novel fold configs
    \item Outcome: Understanding of fold-aerodynamic principles
\end{enumerate}

\subsection{Why This Comparison Matters}

\textit{No single method is ``best''—the choice depends on your goal}:

\begin{itemize}
    \item Need speed? Surrogate + DDPG in 2 minutes.
    \item Need accuracy? FluidX3D on top 10 designs.
    \item Need understanding? GNN + attention analysis.
    \item Need production-ready? Hybrid method.
    \item Need to publish? GNN insights + FluidX3D validation.
\end{itemize}

This flexibility is a key contribution of our framework: practitioners can trade off speed, accuracy, interpretability, and compute based on their constraints.

\section{Conclusion}

This work demonstrates that combining GNNs, GPU-native CFD (FluidX3D), and RL yields a powerful, flexible platform for aerodynamic design exploration. By offering three complementary evaluation strategies—fast surrogate, high-fidelity CFD, and hybrid multi-fidelity—we enable exploration across the speed-accuracy frontier. By learning directly from mesh geometry, GNNs bypass the brittleness of hand-derived surrogates, enabling discovery of counter-intuitive designs while providing interpretable insights. The shift from Docker-based OpenFOAM to Windows-native FluidX3D dramatically improves accessibility and iteration speed, democratizing CFD research to the desktop. While paper airplanes may seem whimsical, the framework scales to real aircraft: imagine a design system where engineers describe high-level goals (range, load capacity, stability) and a flexible multi-fidelity AI system autonomously discovers folding/shaping patterns—all validated by LB-CFD on a GPU cluster. The next regime of aerodynamics may belong not to engineers and wind tunnels, but to AI systems trained to see what we cannot.
