...
setup_replacement = """    def _setup_physics_constants(self):
        ...
        # Consistent lattice velocity for c_s = 1/sqrt(3)
        self.u_lat = self.config.mach_number / math.sqrt(3.0)
        if self.u_lat > 0.15:
            print(f"WARNING: u_lat {self.u_lat:.3f} high, capping at 0.15")
            self.u_lat = 0.15

        # nu = u * L / Re. Using resolution as reference length in lattice units.
        self.nu = self.u_lat * self.resolution / max(float(Re), 1e-12)
        self.tau = 3.0 * self.nu + 0.5
        self.tau = max(self.tau, 0.501) # Stability limit
        self.phys_config.s_nu = 1.0 / self.tau\n"""
content, _ = replace_block(content, '    def _setup_physics_constants(self):', '    def _setup_d3q27_lattice(self):', setup_replacement)
...
content, _ = replace_block(content, '    def _setup_d3q27_lattice(self):', '    def _compute_strain_rate_tensor(self, ux, uy, uz, gradients=None):', lattice_replacement)
